...
def reset_inputs():
    ...
    for item in total_items:
        st.session_state[f"check_{item}"] = "양호"
    # 상세 내역 및 기타 상태 초기화
    st.session_state["issue_detail_key"] = ""
    # 사진 촬영 체크박스도 끄고 싶다면 추가 가능
...
# 건물이나 층수가 바뀌면 reset_inputs 함수가 실행되도록 설정
selected_bldg = st.sidebar.selectbox("건물 선택", list(building_data.keys()), on_change=reset_inputs)
selected_floor = st.sidebar.selectbox("층수 선택", building_data[selected_bldg], on_change=reset_inputs)
...
for idx, item in enumerate(total_items):
    with cols[idx % 3]:
        # [수정] key를 지정하여 세션 상태에서 제어할 수 있도록 함
...
with col_img:
    st.header("📸 현장 사진 첨부")
    show_camera = st.checkbox("📷 사진 촬영 기능 켜기", key="camera_on")
    img_file = st.camera_input("불량 항목 사진 촬영") if show_camera else None
with col_txt:
    st.header("📝 지적 내역 및 비고")
    # [수정] key를 지정하여 세션 상태에서 제어할 수 있도록 함
    issue_detail = st.text_area("상세 불량 사유 입력", height=150, key="issue_detail_key")
...
if st.button("📊 점검 결과 저장 및 구글 시트 전송", use_container_width=True):
    ...
    if sheet:
        try:
            ...
            st.success(f"✅ {full_location} 결과가 성공적으로 저장되었습니다!")
            st.balloons()
            # 저장 후 자동으로 초기화하고 싶다면 여기서도 reset_inputs()를 호출할 수 있습니다.
        except Exception as e:
